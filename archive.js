...
function render(editions, q='', price){
  ...
  list.innerHTML=rows.length?rows.map(e=>`
    <article class="edition-entry">
      ...
      <div><h3>${e.year} · ${e.number}</h3><p>${e.summary}</p>
        <p class="exchange-rate-note">${price.note}</p><div class="edition-meta"><span>${e.priceEur}</span><span>${price.label}</span><span>${e.articleIds.length} artikelen</span></div>
      </div>
...
(async()=>{
  try{
    ...
    render(editions, '', price);
    document.getElementById('archiveSearch').addEventListener('input',e=>render(editions,e.target.value,price));
  }catch(e){
